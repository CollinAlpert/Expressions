...
public abstract class Expression {
	static private final Map<Method, Class<?>> _boxers;
	static private final Map<Method, Class<?>> _unboxers;
	...
	static {
		...
		try {
			_unboxers.put(Boolean.class.getMethod("booleanValue"), Boolean.TYPE);
			_unboxers.put(Byte.class.getMethod("byteValue"), Byte.TYPE);
			_unboxers.put(Character.class.getMethod("charValue"), Character.TYPE);
			_unboxers.put(Double.class.getMethod("doubleValue"), Double.TYPE);
			_unboxers.put(Float.class.getMethod("floatValue"), Float.TYPE);
			_unboxers.put(Integer.class.getMethod("intValue"), Integer.TYPE);
			_unboxers.put(Long.class.getMethod("longValue"), Long.TYPE);
			_unboxers.put(Short.class.getMethod("shortValue"), Short.TYPE);
	 ...
	 * @param resultType     The {@link Class} to set as the type of the expression that this Expression represents.
	 */
	...
	public static Expression exclusiveOr(Expression first, Expression second) {
		if (second.getExpressionType() == ExpressionType.Constant) {
			ConstantExpression constantExpression = (ConstantExpression) second;
			if (isIntegral(constantExpression.getResultType())) {
				if (((Number) constantExpression.getValue()).intValue() == -1) {
	 ...
	 * @param to The {@link Class} to set as the type of the expression that this Expression represents.
	 ...
	 *
	 * @param e    An {@link Expression} to set the getFirst() method equal to.
	 * @param type The {@link Expression} that evaluates to Class assignability is tested with.
	 * @return A {@link BinaryExpression} that has the getExpressionType() equal to InstanceOf, the getFirst() set to 'e'
	 * and getSecond() set to {@link ConstantExpression} with value equals to 'type'.
	 */
	 ...
	 *
	 * @param expressionType The {@link ExpressionType} that specifies the type of unary operation.
	 * @param resultType     The {@link Class} that specifies the type to be converted to (pass null if not applicable).
	 ...
	 *
	 * @param expressionType The {@link ExpressionType} that specifies the type of binary operation.
	 * @param first          An {@link Expression} that represents the left operand.
	 * @param second         An {@link Expression} that represents the right operand.
	 * @return The {@link Expression} that results from calling the appropriate factory method.
	 */
	 ...
	 *
	 * @param resultType The method return value.
	 * @param delegate   The method implementation. The delegate resultType must be {@link InvocableExpression}.
	 * @param parameters The method parameters.
	 ...
	 *
	 * @param type The {@link Class} that specifies the type that contains the specified static field.
	 ...
	 *
	 * @param instance An {@link Expression} whose {@code getResultType()} value will be searched for a specific field.
	 * @param name     The name of a field.
	 ...
	 *
	 * @param expressionType Type of access.
	 * @param instance       An {@link Expression} representing the instance.
	 * @param member         The {@code Member} to be accessed.
	 * @param resultType     The return value type.
	 ...
	 *
	 * @param instance An {@link Expression} representing the instance.
	 * @param field    A field to be accessed.
	...
	public static MemberExpression get(Expression instance, Field field) {
		return member(ExpressionType.FieldAccess, instance, field, field.getType(), Collections.emptyList());
	}
	 ...
	 * case of boxing.
	 *
	 * @param instance  An {@link Expression} whose {@code getResultType()} value will be searched for a specific method.
	 * @param method    The {@link Method} to be called.
	 * @param arguments An array of {@link Expression} objects that represent the arguments to the method.
	...
	public static Expression invoke(Expression instance, Method method, Expression... arguments) {
		return invoke(instance, method, List.of(arguments));
	}
	...
	public static Expression invoke(Expression instance, Method method, List<Expression> arguments) {
		...
		if (method.isSynthetic()) {
			var actualInstance = instance != null ? instance.accept(Interpreter.Instance).apply(null) : null;
			var lambdaExpression = ExpressionClassCracker.getInstance().lambdaFromFileSystem(actualInstance, method);
			return invoke(lambdaExpression, arguments);
		}
	 ...
	 *
	 * @param method    An {@link InvocableExpression} which encapsulates method to be called.
	 * @param arguments An array of {@link Expression} objects that represent the arguments to the method.
	...
	private static List<ParameterExpression> getParameters(Member member) {
		...
		List<ParameterExpression> parameterList = new ArrayList<>(params.length);
	...
	public static InvocationExpression newInstance(Constructor<?> method, Expression... arguments) {
		return newInstance(method, List.of(arguments));
	}
	 ...
	 * Creates a {@link InvocationExpression} that represents calling the specified constructor.
	 *
	 * @param type          {@link Class} to be instantiated.
	 ...
	 * @param parameterTypes An array of {@link Class} objects that specify the type of parameters of the method.
	 * @param arguments      An array of {@link Expression} objects that represent the arguments to the method.
	 * @return An {@link InvocationExpression} that has the {@link ExpressionType} method equal to Invoke or Convert in case
	 * of boxing.
	 * @throws NoSuchMethodException if a matching method is not found.
	 ...
	 *
	 * @param type           The {@link Class} that specifies the type that contains the specified static method.
	 * @param name           The name of the method.
	 * @param parameterTypes An array of {@link Class} objects that specify the type of parameters of the method.
	 * @param arguments      An array of {@link Expression} objects that represent the arguments to the method.
	 * @return An {@link InvocationExpression} that has the {@link ExpressionType} method equal to Invoke or Convert in case
	 ...
	 *
	 * @param clazz          Class which is the start of the search
	 ...
	 * constant.
	 *
	 * @param test    An Expression to set the getOperator() method equal to.
	 * @param ifTrue  An {@link Expression} to set the getFirst() method equal to.
	 * @param ifFalse An {@link Expression} to set the getSecond() method equal to.
	...
	public static Expression condition(Expression test, Expression ifTrue, Expression ifFalse) {
		...
		if ((ifTrue.isBoolean())) {
			...
			if (ifTrueStripped.getExpressionType() == ExpressionType.Constant && ifFalseStripped.getExpressionType() == ExpressionType.Constant) {
				...
				return convert((Boolean) firstConstant.getValue() ? test : Expression.logicalNot(test), ifTrue.getResultType());
			}
		}
	 ...
	 * @param v   The visitor to visit this node with.
	 * @return T
	 */
